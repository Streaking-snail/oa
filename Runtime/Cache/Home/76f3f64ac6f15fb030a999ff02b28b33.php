...
<?php } ?>
												...
												<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample-table-2_info">
													<thead>
														   <tr>
																...
																<?php foreach($malls as $value){ ?>
																	<th width="8%"><?php echo ($value["name"]); ?></th>
																<?php } ?>
															</tr>
													</thead>
	
													<tbody>
														<?php foreach($count_infos as $k=>$value){ ?>
															<tr class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>">
																...
																<td>
																	...
																	<a href="<?php echo U('Download/index',array('file'=>$value['pic_path']));?>">下载</a>
																</td>
																<td>
																	<?php $product_order = M('product_order')->where("product_id=%d", array($value['product_id']))->find(); $deliver_num = D("ProductOrder")->get_deliver_num($product_order['id']); echo $deliver_num; ?>
																</td>
																<td>
																	<?php $deliver_time = D("CountInfo")->get_deliver_time($product_order['id']); echo date("Y-m-d H:i:s", $deliver_time); ?>
																</td>
																<td>
																	<?php $finish_time = D('CountInfo')->get_end_time($value['product_id']); echo date("Y-m-d H:i:s", $finish_time); ?>
																</td>
																<?php foreach($malls as $v){ ?>
																	<td>
																		<!-- 
																		<a href="<?php echo U('ShopCount/index', array('mall_id'=>$v['id'], 'product_id'=>$value['product_id']));?>">
																			<?php echo D('CountInfo')->get_count($value['product_id'], $v['id']); ?>
																		</a>
																		-->
																		<?php $shops = D('CountInfo')->get_shops($v['id'], $value['product_id']); if( count($shops)>0 ){ foreach($shops as $val){ if( $val['is_over'] ){ echo "<span style='color:green;'>".$val['name']."</span>"; }else{ echo "<span style='color:red;'>".$val['name']."</span>"; } } }else{ echo ""; } ?>
																	</td>
																<?php } ?>
															</tr>
														<?php } if( count($count_infos)==0 ){ echo "<tr><td align='center' colspan='".(count($malls)+7)."'>暂无相关记录</td></tr>"; } ?>
													</tbody>
												</table>
												<div class="row">
		...
		<a href="<?php echo U('CountInfo/download_csv');?>" title="下载表格">
                    下载表格    
                </a>
