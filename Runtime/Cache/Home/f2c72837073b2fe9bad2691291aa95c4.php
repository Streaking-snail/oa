			...
			<label>
				...
				<?php } ?>
								...
								<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample-table-2_info">
									...
									<tbody>
										<?php foreach($deports as $k=>$value){ ?>
											<tr role="row" class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>">
												<td>
													<a href="<?php echo U('Home/AdminUser/index',array('deport_id'=>$value['id']));?>"><?php echo ($value["name"]); ?></a>
												</td>
												<td><?php echo ($value["content"]); ?></td>
												<td class=""><?php echo date("Y-m-d H:i:s", $value['create_time']); ?></td>
												<td><?php echo $Deport->role_list($value['id']); ?></td>
												<td><?php echo $Deport->user_list($value['id']); ?></td>
												<td><?php echo ($value["rank"]); ?></td>
												<td>
													<div class="hidden-sm hidden-xs btn-group">
														<a href="<?php echo U('Deport/edit', array('id'=>$value['id']));?>">
															<button class="btn btn-xs btn-info">
																<i class="ace-icon fa fa-pencil bigger-120"></i>
															</button>
														</a>
														<a href="<?php echo U('Deport/delete_deport', array('id'=>$value['id']));?>">
										...
										<?php } ?>
										<?php if( count($deports)==0 ){ echo "<tr><td colspan='6'>暂无相关记录</td></tr>";} ?>
									</tbody>
								</table>
								<div class="row">
