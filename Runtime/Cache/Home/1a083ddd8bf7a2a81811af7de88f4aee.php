...
<ul class="nav nav-list">
	...
	<h1>
		...
		<div class="row">
				<div class="col-xs-12">
								<?php if( !empty($search_info['key']) ){ ?>
	<div class="table-header">
			当前搜索"<?php echo ($search_info["key"]); ?>"
	</div>
<?php } ?>
				...
				<?php } ?>
                                ...
                                <?php } ?>
												...
												<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample-table-2_info">
													...
													<tbody>
														<?php foreach($admin_users as $k=>$value){ ?>
															<tr role="row" class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>"> 
																<td class="center">
																	<?php echo ($k+1+$page_number); ?>
																</td>
																<td><?php echo ($value["username"]); ?></td>
																<td>
																	<a href="<?php echo U('Home/AdminUser/edit', array('id'=>$value['id']));?>"><?php echo ($value["name"]); ?></a>
																</td>
																<td>
																	<?php echo $AdminUser->deport_name($value['deport_id']) ?>
																</td>
																<td>
																	<?php echo $AdminUser->role_name($value['role_id']) ?>
																</td>
																...
																<td class="hidden-480 sorting_1">
																	<?php if($value['status']==1){ ?>
																		<span class="label label-sm">正常</span>
																	<?php }else if( $value['status']==2 ){ ?>
																		<span class="label label-sm label-warning">锁定</span>
																	<?php } ?>
																	<?php if($value['is_test']==1){ ?>
																	<br />测试账号
																	<?php } ?>
																</td>
		
																<td>
																	<?php if( $value['id']!=1 || ($value['id']==$admin_user_id && $admin_user_id==1) ){ ?>
																		<div class="hidden-sm hidden-xs action-buttons">
																			<!--  
																			<a class="blue" href="<?php echo U('AdminUser/detail', array('id'=>$value['id']));?>" title="查看详情">
																				<i class="ace-icon fa fa-search-plus bigger-130"></i>
																			</a>
																			-->
																			<?php if( $value['status']==2 ){ ?>	
																				<a href="<?php echo U('AdminUser/unlock', array('id'=>$value['id']));?>" title="解锁">
																					<button class="btn btn-xs btn-success">
																						<i class="ace-icon fa fa-check bigger-120"></i>
																					</button>
																				</a>
																			<?php }else if( $value['status']==1 ){ ?>
																				<a href="<?php echo U('AdminUser/lock', array('id'=>$value['id']));?>" title="锁定">
																					<button class="btn btn-xs btn-warning">
																						<i class="ace-icon fa fa-flag bigger-120"></i>
																					</button>
																				</a>
																			<?php } ?>
																			<a href="<?php echo U('AdminUser/edit', array('id'=>$value['id']));?>" title="修改">
																				<button class="btn btn-xs btn-info">
																					<i class="ace-icon fa fa-pencil bigger-120"></i>
																				</button>
																			</a>
																			<a href="<?php echo U('AdminUser/delete_user', array('id'=>$value['id']));?>" title="删除">
																				<button class="btn btn-xs btn-danger">
																					<i class="ace-icon fa fa-trash-o bigger-120"></i>
																				</button>
																			</a>
																			<?php if( $value['is_test']==1 ){ ?>
																			<a href="<?php echo U('AdminUser/delete_test_user_info', array('id'=>$value['id']));?>" title="删除">
																				删除测试数据
																			</a>
																			<?php } ?>
																		</div>
														...
														<?php } ?>
														<?php if( count($admin_users)==0 ){ echo "<tr><td colspan='8' style='text-align:center;line-height:30px;'>暂无相关记录</td></tr>"; } ?>
													</tbody>
												</table>
												<div class="row">
