		...
		<div class="row">
				<div class="col-xs-12">
							<div id="sample-table-2_wrapper" class="dataTables_wrapper form-inline no-footer" style="overflow-x:auto;">
												<input type="hidden" id="add_url" value="<?php echo U('Temporary/add_info');?>" />
												<input type="hidden" id="edit_url" value="<?php echo U('Temporary/edit');?>" />
												<input type="hidden" id="show_url" value="<?php echo U('Temporary/detail');?>" />
												<div class="row">
...
<!--            <?php if($business_search){ ?>
    ...
    <input id="search" type="submit" value="搜索" />
												...
												<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample-table-2_info">
													...
													<tbody>
														<?php foreach($temporaries as $k=>$value){ $s = D('Temporaries')->get_local_status($value['id']); $t = D('Temporaries')->get_status($value['id']); ?>
															<tr data-id="<?php echo ($value["id"]); ?>" class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>" name="item">
																<td><?php echo ($k+1); ?></td>
																<td><?php echo ($value["no"]); ?></td>
																<td>
																	<a href="javascript:void(0);" name="show_status" data-url="<?php echo U('Temporary/get_checked_list', array('id'=>$value['id']));?>">
																...
																<td><?php echo ($value["deport_name"]); ?></td>
																<td><?php echo date("Y-m-d", $value['report_time']); ?></td>
																<td><?php echo ($value["content"]); ?></td>
																<td><?php echo D('AdminUser')->get_deport_name($value['admin_user_id']); ?></td>
																<td><?php echo ($value["name"]); ?></td>
																<td><?php echo date("Y-m-d", $value['create_time']); ?></td>
																<td><?php echo $Temoraries->get_last_checked_name($value['id']); ?></td>
																<td>
																	<a name="show_info" href="<?php echo U('Temporary/detail', array('id'=>$value['id']));?>" target="_blank" title="详情">
																		详情
																	</a>&nbsp;|
																	<?php if( $value['is_over']==0 ){ ?>
																		<a href="<?php echo U('Temporary/edit', array('id'=>$value['id']));?>" title="修改">
																			修改
																		</a>&nbsp;|
																		<?php if( D('Temporaries')->is_checked_power($user_info['id'], $value['id']) ){ ?>
																	...
																	<?php if( $value['admin_user_id']==$user_info['id'] ){ ?>
																		<a href="<?php echo U('Temporary/del', array('id'=>$value['id']));?>" title="删除">
																			删除
																		</a>
																	<?php } ?>
																</td>
															</tr>
														<?php } if( count($temporaries)==0 ){ echo "<tr><td align='center' colspan='13'>暂无相关记录</td></tr>"; } ?>
													</tbody>
												</table>
												<div class="row">
