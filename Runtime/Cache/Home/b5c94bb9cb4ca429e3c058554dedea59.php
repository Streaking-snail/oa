													...
													<tbody>
														<?php foreach($products as $k=>$value){ ?>
															<tr class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>" name="item" vel="<?php echo ($value["id"]); ?>">
																...
																<td><?php echo $Product->get_category_name($value['category_id']); ?></td>
																<td><?php echo $Product->get_sub_category_name($value['sub_category_id']); ?></td>
																<td><?php echo $Product->get_classification_name($value['classification_id']); ?></td>
																<td><?php echo ($value["code"]); ?></td>
																<td><?php echo ($value["number"]); ?></td>
																<td>
																	<img src="<?php echo C('WEBURL').$value['pic_path']; ?>" width="50" height="50" />
																	<?php echo ($value["name"]); ?><br />
																	<a href="<?php echo U('Download/index',array('file'=>urlencode($value['pic_path'])));?>">下载</a>
																</td>
																<td><?php echo ($value["now_price"]); ?></td>
																<td><?php echo ($value["sold_price"]); ?></td>
																<td><?php echo ($value["min_price"]); ?></td>
																<td><?php echo ($value["min_num"]); ?></td>
																<td><?php echo ($value["days"]); ?></td>
																<td><?php if(!empty($value['ptime'])){ echo date("Y-m-d", $value['ptime']); } ?></td>
																<td><?php echo ($value["content"]); ?></td>
																<td>
																	<?php if($value['status']==1){ echo "<span class='label label-sm label-success'>审核通过</span>"; }elseif( $value['status']==2 ){ echo "<span class='label label-sm label-inverse arrowed-in'>审核不通过</span>"; }else{ echo "<span class='label label-sm label-warning'>未审核</span>"; } ?>
																</td>
																<td>
																	<div class="hidden-sm hidden-xs action-buttons">
																		<?php if( $value['status']==0 && D('Roles')->is_power('Product', 'check', $user_info['id']) ){ ?>
																		<a class="blue" href="<?php echo U('Product/checked', array('id'=>$value['id']));?>" title="审核通过">
																			<i class="ace-icon fa fa-check bigger-120"></i>
																		</a>
																		
																		<a class="blue" href="<?php echo U('Product/unchecked', array('id'=>$value['id']));?>" title="审核不通过">
																			<i class="ace-icon fa fa-flag bigger-120"></i>
																		</a>
																		<?php } ?>
																		
																		<?php if( D('Roles')->is_power('Product', 'edit', $user_info['id']) ){ ?>
																		<a class="green" href="<?php echo U('Product/edit', array('id'=>$value['id']));?>" title="修改">
																			<i class="ace-icon fa fa-pencil bigger-130"></i>
																		</a>
																		<?php } ?>
																		<?php if( $value['status']==0 ){ ?>
																		<a class="red" href="<?php echo U('Product/del', array('id'=>$value['id']));?>" title="删除">
															...
															<tr id="item_<?php echo ($value["id"]); ?>" data-s="1" style="display:none;">
																<td colspan="15">
																	<?php $product_logs=D('AdminUserLog')->get_product_logs($value['id']); ?>
																	<table width="100%" border=1>
																		<tr>
																		<th width="50%">旧数据</th>
																		<th width="50%">修改数据</th>
																		</tr>
																		<?php foreach($product_logs as $v){ $res = json_decode($v['content']); ?>
																			<tr>
																			<td>
																			现价:<?php echo ($res->oinfo->now_price); ?>&nbsp;统一售价：<?php echo ($res->oinfo->sold_price); ?>&nbsp;最低售价：<?php echo ($res->oinfo->min_price); ?>
																			起订量：<?php echo ($res->oinfo->min_num); ?>&nbsp;生产周期：<?php echo ($res->oinfo->days); ?>&nbsp;计划完成时间：<?php echo date("Y-m-d H:i:s", $res->oinfo->ptime); ?>&nbsp;
																			创建者：<?php echo D('AdminUser')->get_name($res->oinfo->admin_user_id) ?>&nbsp;&nbsp;创建时间：<?php echo date("Y-m-d H:i:s", $res->oinfo->create_time) ?>
																			</td>
																			<td>
																			现价:<?php echo ($res->ninfo->now_price); ?>&nbsp;统一售价：<?php echo ($res->ninfo->sold_price); ?>&nbsp;最低售价：<?php echo ($res->ninfo->min_price); ?>
																			起订量：<?php echo ($res->ninfo->min_num); ?>&nbsp;生产周期：<?php echo ($res->ninfo->days); ?>&nbsp;计划完成时间：<?php echo ($res->ninfo->ptime); ?></php>&nbsp;
																			修改者：<?php echo D('AdminUser')->get_name($v['admin_user_id']) ?>&nbsp;&nbsp;修改时间：<?php echo date("Y-m-d H:i:s", $v['create_time']) ?>
																			</td>
																			</tr>
																		<?php } ?>
																	</table>
														...
														<?php } if( count($products)==0 ){ echo "<tr><td align='center' colspan='15'>暂无相关记录</td></tr>"; } ?>
													</tbody>
												</table>
												<div class="row">
