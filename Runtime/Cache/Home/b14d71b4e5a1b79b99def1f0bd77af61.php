...
<?php } ?>
												...
												<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample-table-2_info">
													...
													<tbody>
														<?php foreach($process_types as $k=>$value){ ?>
															<tr class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>" name="item" vel="<?php echo ($value["id"]); ?>">
                                                                <td><?php echo ($page_number+$k+1); ?></td>
																<td><?php echo ($value["name"]); ?></td>
																<td>
																	<?php $process_type_status = $ProcessType->get_status($value['id']); foreach($process_type_status as $k=>$v){ echo $v['name']; if( count($process_type_status)-1!=$k ){ echo "->"; } } ?>
																</td>
																<td><?php echo ($value["content"]); ?></td>
																<td>
																	<?php if( $user_info['is_admin']==1 ){ ?>
																		<a href="<?php echo U('ProcessType/edit', array('id'=>$value['id']));?>">修改</a>
																	<?php } ?>
																</td>
															</tr>
														<?php } if( count($process_types)==0 ){ echo "<tr><td align='center' colspan='5'>暂无相关记录</td></tr>"; } ?>
													</tbody>
												</table>
												<div class="row">
