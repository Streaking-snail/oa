...
<div class="page-content-area">
	...
	<div class="row">
		<div class="col-xs-12">
			<form class="form-horizontal" action="<?php echo U('CountInfo/create_info');?>" method="post" role="form">
...
<input type="hidden" name="count_info[product_id]" id="count_info_product_id" value="<?php echo ($count_info["product_id"]); ?>"/>
<div class="form-group">
	<label class="col-sm-3 control-label no-padding-right" for="form-field-5"><span class="err">*</span>产品名称/辅代码/物料代码：<br />(只有图片流程完成才能添加)</label>

	<div class="col-sm-9">
		<input type="text" id="product_name" name="product_name" data-url="<?php echo U('CountInfo/get_products');?>" placeholder="产品名称/辅代码/物料代码" class="col-xs-10 col-sm-5" value="<?php echo ($count_info["product_name"]); ?>" />
		<span class="help-inline col-xs-12 col-sm-7">
			<label class="middle">
				<span class="lbl">请输入产品!</span>
			</label>
		</span>
	</div>
</div>
...
<div class="form-group">
	<label class="col-sm-3 control-label no-padding-right" for="form-field-5"><span class="err">*</span>商城：</label>

	<div class="col-sm-9">
		<select name="count_info[mall_id]" id="count_info_mall_id" class="col-xs-10 col-sm-5" data-url="<?php echo U('CountInfo/select_shops');?>">
			<?php foreach( $malls as $k=>$value ){ if( intval($value['id'])==intval($count_info['mall_id']) ){ echo '<option value="'.$value['id'].'" selected="selected">'.$value['name'].'</option>'; }else{ echo '<option value="'.$value['id'].'">'.$value['name'].'</option>'; } } ?>
		</select>
		<span class="help-inline col-xs-12 col-sm-7">
			<label class="middle">
				<span class="lbl">请选择商城!</span>
			</label>
		</span>
	</div>
</div>
...
<div class="form-group">
	<label class="col-sm-3 control-label no-padding-right" for="form-field-5"><span class="err">*</span>店铺：</label>
	<div class="col-sm-9" id="shop_list">
		<?php if( count($shops)>0 ){ ?>
	<?php foreach($shops as $k=>$value){ ?>
		<input type="checkbox" name="shop_ids[]" checked="checked" id="shops_<?php echo ($k); ?>_id" value="<?php echo ($value["id"]); ?>" /> <?php echo ($value["name"]); ?>
	<?php } ?>
	<span class="help-inline col-xs-12 col-sm-7">
		<label class="middle">
			<span class="lbl">请选择店铺!</span>
		</label>
	</span>
<?php }else{ ?>
	<span>
		<label class="middle">
			<span class="lbl" style="display:block;">请先去&nbsp;<a href="<?php echo U('Shop/add_info');?>">添加店铺</a></span>
		</label>
	</span>
<?php } ?>
	</div>
