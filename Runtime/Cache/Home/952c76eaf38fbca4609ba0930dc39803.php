								...
								<tbody>
									<?php foreach($roles as $k=>$value){ ?>
										<tr role="row" class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>">
											<td>
												<?php echo $Role->deport_name($value['deport_id']); ?>
											</td>
											<td><?php echo ($value["name"]); ?></td>
											<td><?php echo ($value["content"]); ?></td>
											<td>
												<?php echo $Role->get_permession_list($value['id']); ?>
											</td>
											<td class=""><?php echo date("Y-m-d H:i:s", $value['create_time']); ?></td>
					
											<td>
												<div class="hidden-sm hidden-xs btn-group">
													<a href="<?php echo U('Roles/edit', array('id'=>$value['id']));?>">
														<button class="btn btn-xs btn-info">
															<i class="ace-icon fa fa-pencil bigger-120"></i>
														</button>
													</a>
													<a href="<?php echo U('Roles/delete_role', array('id'=>$value['id']));?>">
