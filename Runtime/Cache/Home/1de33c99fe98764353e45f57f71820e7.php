...
<input type="hidden" name='show' id="show" value="<?php echo ($search_info['show']); ?>" />
    ...
    <input id="search" type="submit" value="搜索" />
												...
												<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample-table-2_info">
													...
													<tbody>
														<?php foreach($product_orders as $k=>$value){ $deliver_num = D("ProductOrder")->get_deliver_num($value['id']); $deliver_time = D("ProductOrder")->get_deliver_time($value['id']); ?>
															<tr class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>" name="item" vel="<?php echo ($value["id"]); ?>">
                                                                <td><?php echo ($page_number+$k+1); ?></td>
																<?php $p = M('products')->where("id=%d", array($value['product_id']))->find(); ?>
																<td><?php echo ($value["code"]); ?></td>
																<td><?php echo ($value["number"]); ?></td>
																<td><?php echo ($value["name"]); ?></td>
																<td>
																<?php if( !empty($value['order_time']) ){ $f = true; echo date("Y-m-d H:i:s", $value['order_time']); }else{ $f = false; echo $ProductOrder->get_show_time($value['update_time']); } ?>
																</td>
																<td><?php echo ($value["jd_num"]); ?></td>
																<td><?php echo ($value["tm_num"]); ?></td>
																<td><?php echo ($value["tb_num"]); ?></td>
																<td><?php echo ($value["ka_num"]); ?></td>
																<td><?php echo ($value["sc_num"]); ?></td>
																<td><?php echo $value['jd_num']+$value['tm_num']+$value['tb_num']+$value['ka_num']+$value['sc_num']; ?></td>
																<td>
																	<?php if(!empty($value['deliver_time'])){ $f = true; echo date("Y-m-d H:i:s", $value['deliver_time']); }else if( $f ){ $f = false; echo $ProductOrder->get_show_time($value['update_time']); } ?>
																</td>
																
																<td>
																	<?php if( !empty($deliver_time) ){ echo date("Y-m-d H:i:s", $deliver_time); } ?>
																</td>
																<td>
<!--																	<?php if(!empty($value['deliver_time']) && !empty($value['dh_time'])){ echo date("Y-m-d H:i:s", $value['dh_time']); }else if(!empty($value['deliver_time'])){ echo $ProductOrder->get_show_time($value['update_time']); } ?>-->
                                                                    <?php echo ($deliver_num); ?>
																</td>
                                                                <td><?php echo ($value["content"]); ?></td>
																<td>
																    <?php if( $deliver_num==0 && ( $value['admin_user_id']==$user_info['id'] || D('Roles')->is_power('ProductOrder', 'edit', $user_info['id'])) ){ ?>
																		<a href="<?php echo U('ProductOrder/edit', array('id'=>$value['id']));?>" title="修改">
																			<button class="btn btn-xs btn-info">
																				<i class="ace-icon fa fa-pencil bigger-120"></i>
																			</button>
																		</a>
																	<?php }else if($deliver_num<($value['jd_num']+$value['tm_num']+$value['tb_num']+$value['ka_num']+$value['sc_num'])){ ?>
																		<span class="label label-sm label-inverse">到货数不足</span>
																		<?php if( $value['admin_user_id']==$user_info['id'] || D('Roles')->is_power('ProductOrder', 'edit', $user_info['id']) ){ ?>
																			<a href="<?php echo U('ProductOrder/add_deliver_num', array('id'=>$value['id']));?>" title="添加到货数">
																				添加到货数
																			</a>
																		<?php } ?>
																	<?php }else{ ?>
																	...
																	<?php } ?>
																	<?php if( $deliver_num==0 && (D('Roles')->is_power('ProductOrder', "del", $user_info['id']) || $value['admin_user_id']==$user_info['id']) ){ ?>
																		<a href="<?php echo U('ProductOrder/del', array('id'=>$value['id']));?>" title="删除">
															...
															<tr style="display:none;" id="item_<?php echo ($value["id"]); ?>" data-s="0">
																<td colspan="3">
																	提交者：<?php echo D('AdminUser')->get_name($value['admin_user_id']); ?><br />
																	提交时间：<?php echo date("Y-m-d H:i:s", $value['create_time']); ?>
																</td>
																<td colspan="13">
																		<?php $order_delivers = M('product_order_deliver')->where("product_order_id=%d", array($value['id']))->select(); foreach($order_delivers as $v){ ?>
																			<p>
																				到货时间：<?php echo date("Y-m-d H:i:s", $v['deliver_time']); ?>&nbsp;&nbsp;
																				到货数量：<?php echo ($v["num"]); ?>&nbsp;&nbsp;
																				备注：<?php if(!empty($v['content'])){echo $v['content'];}else{echo "无";} ?>&nbsp;&nbsp;
																				提交者：<?php echo D('AdminUser')->get_name($v['admin_user_id']); ?>&nbsp;&nbsp;
																				提交时间：<?php echo date("Y-m-d H:i:s", $v['create_time']); ?>&nbsp;&nbsp;
																			</p>
																		<?php } ?>
																</td>
															</tr>
														<?php } if( count($product_orders)==0 ){ echo "<tr><td align='center' colspan='16'>暂无相关记录</td></tr>"; } ?>
													</tbody>
												</table>
												<div class="row">
