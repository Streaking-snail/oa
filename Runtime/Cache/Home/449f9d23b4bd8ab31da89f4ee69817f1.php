												...
												<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample-table-2_info">
													...
													<tbody>
														<?php foreach($s_menus as $k=>$value){ ?>
															<tr role="row" name="item" vel="<?php echo ($value["id"]); ?>" data-s="0" class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>">
																<td><?php echo ($value["name"]); ?></td>
																<td><?php echo ($value["url"]); ?></td>
																<td><?php echo ($value["content"]); ?></td>
																<td><a href="<?php echo U('Menus/edit', array('id'=>$value['id']));?>">修改</a></td>
															</tr>
															<?php foreach($value['sub_menus'] as $val){ ?>
																<tr id="item_<?php echo ($val["id"]); ?>" class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>">
																	...
																	<td><a href="<?php echo U('Menus/edit', array('id'=>$val['id']));?>">修改</a></td>
																</tr>
																<?php foreach($val['sub_menus'] as $v){ ?>
																	<tr id="item_<?php echo ($val["id"]); ?>" class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>">
																		...
																		<td><a href="<?php echo U('Menus/edit', array('id'=>$v['id']));?>">修改</a></td>
																	</tr>
																<?php } ?>
																
															<?php } ?>
														<?php } if( count($s_menus)==0 ){ echo "<tr><td align='center' colspan='".(3+count($s_status))."'>暂无相关记录</td></tr>"; } ?>
													</tbody>
												</table>
												<div class="row">
