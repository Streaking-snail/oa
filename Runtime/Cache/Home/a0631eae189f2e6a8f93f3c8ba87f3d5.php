...
<input type="hidden" name='show' id="show" value="<?php echo ($search_info['show']); ?>" />
    物料代码：<input type="text" name="code" value="<?php echo ($search_info['code']); ?>" />
    辅代码：<input type="text" name="number" value="<?php echo ($search_info['number']); ?>" />
    产品名称：<input type="text" name="name" value="<?php echo ($search_info['name']); ?>" />
    
    <?php if($is_develop){ ?>
        完成时间：
        <input type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="ptime1" value="<?php if( !empty($search_info['ptime1']) )echo date('Y-m-d H:i:s', $search_info['ptime1']); ?>" />
        -<input type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="ptime2" value="<?php if( !empty($search_info['ptime1']) )echo date('Y-m-d H:i:s', $search_info['ptime1']); ?>" />
    <?php } ?>
    ...
    <select name="time">
        ...
        <option value="dh_time">到货时间:</option>
        <input type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="time1" value="<?php if( !empty($search_info['time1']) )echo date('Y-m-d H:i:s', $search_info['time1']); ?>" />
        -<input type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="time2" value="<?php if( !empty($search_info['time2']) )echo date('Y-m-d H:i:s', $search_info['time2']); ?>" />
    <?php } ?>
    
    <?php if($is_price || $is_develop){ ?>
    <select name="price">
        <option <?php if( $search_info['price'] == new_price ) echo selected; ?> value="now_price">现价</option>
        <option <?php if( $search_info['price'] == sold_price ) echo selected; ?> value="sold_price">统一售价</option>
        <option <?php if( $search_info['price'] == min_price ) echo selected; ?> value="min_price">最低售价</option>
    </select>
    <input type="text" name="price1" value="<?php echo ($search_info['price1']); ?>" />-<input type="text" name="price2" value="<?php echo ($search_info['price2']); ?>" />
    <?php } ?>
    ...
    <select name="pic">
        <option value="">请选择</option>
        <option <?php if( $search_info['pic'] == '1' ) echo selected; ?> value="1">交样</option>
        <option <?php if( $search_info['pic'] == '2' ) echo selected; ?> value="2">文案</option>
        <option <?php if( $search_info['pic'] == '3' ) echo selected; ?> value="3">方案审核</option>
        <option <?php if( $search_info['pic'] == '4' ) echo selected; ?> value="4">拍照</option>
        <option <?php if( $search_info['pic'] == '5' ) echo selected; ?> value="5">详情页制作</option>
        <option <?php if( $search_info['pic'] == '6' ) echo selected; ?> value="6">详情页审核</option>
        <option <?php if( $search_info['pic'] == '7' ) echo selected; ?> value="7">首页制作</option>
        <option <?php if( $search_info['pic'] == '8' ) echo selected; ?> value="8">首页审核</option>
        <option <?php if( $search_info['pic'] == '9' ) echo selected; ?> value="9">完成</option>
    </select>
    <?php } ?>
    
    <?php if( $is_schedule || $is_price){ ?>
    排序：
    <select name="sort">
        <?php if(is_array($sort_arrs)): foreach($sort_arrs as $key=>$sort_arrs): ?><option <?php if( $search_info['sort'] == $sort_arrs['0'] ) echo selected; ?> value="<?php echo ($sort_arrs['0']); ?>"><?php echo ($sort_arrs['1']); ?></option><?php endforeach; endif; ?>
    </select>
    <?php } ?>
												...
												<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample-table-2_info">
													<thead>
														   ...
														   <tr>
																...
																<?php foreach($ptype_statuses as $value){ ?>
													...
													<tbody>
														<?php $Role = D('Roles'); ?>
														<?php foreach($product_pics as $k=>$value){ ?>
															<tr class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>" name="item" vel="<?php echo ($value["id"]); ?>">
                                                                <td><?php echo ($page_number+$k+1); ?></td>
																<td name='td-item'><a href="javascript:void(0);"><?php echo ($value["code"]); ?></a></td>
																<td name='td-item'><a href="javascript:void(0);"><?php echo ($value["number"]); ?></a></td>
																<td name='td-item'><a href="javascript:void(0);"><?php echo ($value["name"]); ?></a></td>
																<?php foreach($ptype_statuses as $i=>$v){ ?>
																	<td>
																		<?php $r = $ProductPic->get_pic_status($value['id'], $v['id']); if( in_array($r['status'], array(0,2)) && $r['ptype_status_id']!=0 ){ ?>
																	    	<?php if( $Role->text_power($user_info['role_id'], $v['id']) || $Role->attach_power($user_info['role_id'], $v['id']) ){ ?>
                                                                                                                                                     <div class="info_form">
                                                                                                                                                        <div class="title">
                                                                                                                                                             <span class="tit">添加信息</span>
                                                                                                                                                             <span class="closed">[关闭]</span>
                                                                                                                                                        </div>
                                                                                                                                                        <form action="<?php echo U('ProductPic/upload_file');?>"  enctype="multipart/form-data" method="post" >
                                                                                                                                                            <p class="mtl20">       
                                                                                                                                                            <input type="hidden" name="id" id="id" value="<?php echo ($r["id"]); ?>" />
                                                                                                                                                                                                                                           <?php if( $Role->text_power($user_info['role_id'], $v['id']) ){ ?>
                                                                                                                                                                                                                                                       <input type="text" id="content" name="content" placeholder="描述" value="" />&nbsp;
                                                                                                                                                                   <?php } ?>
                                                                                                                                                                   <?php if( $Role->attach_power($user_info['role_id'], $v['id']) ){ ?>
																	    	...
																	    	<?php } ?>
																	    	<?php if( $Role->check_power($user_info['role_id'], $v['id']) ){ ?>
                                                                                                                                                <div style="margin:5px 0;">
                                                                                                                                                    <a href="<?php echo U('ProductPic/update_status', array('sid'=>$v['id'], 't'=>'checked', 'id'=>$value['id']));?>">
                                                                                                                                                        <input type="button" style="border:none;background:green;color:#fff;" value="同意" />
                                                                                                                                                    </a>
                                                                                                                                                    <?php if( $i!=0 ){ ?>
                                                                                                                                                            &nbsp;|&nbsp;<a href="javascript:void(0);" onclick="refuse('<?php echo ($v[id]); ?>','unchecked','<?php echo ($value[id]); ?>')" >
                                                                                                                                                                <input type="button" style="border:none;background:red;color:#fff;" value="拒绝" />
                                                                                                                                                            </a>
                                                                                                                                                    <?php } ?>   <br /> 
                                                                                                                                                </div>
																	    	<?php } echo "所用时间：".D('ProductOrder')->get_show_time($r['create_time'])."<br />"; }else if( $r['status']==1 ){ $user = M('admin_user')->where("id=%d", array($r['checked_user_id']))->find(); if($user){ echo $user['name']." <span style='color:green;'>同意</span> ".date("Y-m-d H:i:s", $r['checked_time'])."<br />"; } } $refuse = D('ProductPic')->getRefuse($v['id'], $value['id']); $re = $refuse[0]; if( $re ){ echo "<p style='width:100px;overflow-x:auto;border:1px solid #c2c2c2;'>"; echo $re['name']; echo " ".date("Y-m-d", $re['create_time'])."<br />"; echo " 拒绝理由:<span style='color:red;'>".$re['content']."</span>"; echo "</p>"; } $attaches = $ProductPic->get_pic_status_attach($value['id'], $v['id']); $attach = $attaches[0]; if( $attach ){ echo $attach['name']." "; if( !empty($attach['content']) ){ echo "  ".$attach['content']; }; if( !empty($attach['url']) ){ $arrs = explode("/", $attach['url']); echo "  <a href='".U('Download/index', array('file'=>urlencode($attach['url'])))."'>".$arrs[count($arrs)-1]."</a>"; $status = $ProductPic->get_status($attach['id']); if($status['status'] != 1 && $attach['admin_user_id']==$_SESSION['admin_user_id']){ echo "  <a href='".U('Download/deletefile', array('id'=>$attach['id']))."'><span style='color:red;'>删除</span></a>"; } } } ?>
																	</td>
																<?php } ?>
																<td>
                                                                                                                                   
																    <?php if( ($value['admin_user_id']==$user_info['id'] || $Role->is_power('ProductOrder', 'del', $user_info['id'])) && $value['is_over']==0 ){ ?>
																		<a href="<?php echo U('ProductPic/del', array('id'=>$value['id']));?>" title="删除">
																			<button class="btn btn-xs btn-danger">
																				<i class="ace-icon fa fa-trash-o bigger-120"></i>
																			</button>
																		</a>                                                                                                                                           
																	<?php }else if($value['is_over']==1){ ?>
																		<span class="label label-sm label-success">已完成</span>
																	<?php } ?>
																</td>
															</tr>
															<tr id="item_<?php echo ($value["id"]); ?>" style="display:none;">
																<td colspan="4">
																	<?php $u=M('admin_user')->where("id=%d", array($value['admin_user_id']))->find(); ?>
																	提交者：<?php echo ($u["name"]); ?>
																	提交时间：<?php echo date("Y-m-d H:i:s", $value['create_time']) ?>
																</td>
																<?php foreach($ptype_statuses as $v){ $attaches = $ProductPic->get_pic_status_attach($value['id'], $v['id']); ?>
																	<td>
                                                                                                                                                <?php $refuse = D('ProductPic')->getRefuse($v['id'], $value['id']); foreach($refuse as $re){ echo "<p style='width:100px;overflow-x:auto;'>"; echo $re['name']; echo " ".date("Y-m-d", $re['create_time']); echo " 拒绝理由:<span style='color:red;'>".$re['content']."</span>"; echo "</p>"; } foreach($attaches as $t){ echo "<p style='width:100px;overflow-x:auto;'>"; echo $t['name']." "; if( !empty($t['content']) ){ echo "  ".$t['content']; }; if( !empty($t['url']) ){ $str_atts = explode("/", $t['url']); echo "  <a href='".U('Download/index', array('file'=>urlencode($t['url'])))."' title='".$str_atts[count($str_atts)-1]."'>".$str_atts[count($str_atts)-1]."</a>"; } echo "</p>"; } ?>
																	</td>
																<?php } ?>
															</tr>
														<?php } if( count($product_pics)==0 ){ echo "<tr><td align='center' colspan='".(5+count($ptype_statuses))."'>暂无相关记录</td></tr>"; } ?>
													</tbody>
												</table>
												<div class="row">
