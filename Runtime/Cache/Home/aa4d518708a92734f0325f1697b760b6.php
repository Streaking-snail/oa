				...
				<?php } ?>
                                ...
                                <?php } ?>
												...
												<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample-table-2_info">
													...
													<tbody>
														<?php foreach($classifications as $k=>$value){ ?>
															<tr role="row" class="<?php if($k%2==0){echo 'odd';}else{echo 'even';} ?>">
																<td><?php echo D('Classification')->get_category_name($value['sub_category_id']); ?></td>
																<td><?php echo D('Product')->get_sub_category_name($value['sub_category_id']); ?></td>
																<td><?php echo ($value["name"]); ?></td>
																<td><?php echo ($value["content"]); ?></td>
																<td><?php echo ($value["rank"]); ?></td>
																<td>
																	<a href="<?php echo U('Classification/edit', array('id'=>$value['id']));?>" title="修改">
																		<button class="btn btn-xs btn-info">
																			<i class="ace-icon fa fa-pencil bigger-120"></i>
																		</button>
																	</a>
																	<a href="<?php echo U('Classification/delete_item', array('id'=>$value['id']));?>" title="删除">
														...
														<?php } if( count($classifications)==0 ){ echo "<tr><td align='center' colspan='6'>暂无相关记录</td></tr>"; } ?>
													</tbody>
												</table>
												<div class="row">
