...
class DoUseController extends AdminBaseController {
	...
	function __construct(){
		...
		$this->locations = array(
				array('is_checked'=>false, 'name'=>'内销办事处领用申请单', 'url'=>U('DoUse/index'))
		);
       $this->assign('loc_name_1', "内销办事处领用申请单");
	...
	public function index(){
		...
		$search_info['show'] = isset($_POST['show'])?$_POST['show']:'0';
		$this->assign('search_info', $search_info);
		$DoUse = D('DoUse');
		$res = $DoUse->search_info($search_info);
		$this->assign('is_number', 1);
		if( $this->user_info['is_admin']==1 || $this->user_info['id']==1){
			$this->assign('application', 1);
		}
		$this->assign('DoUse', $DoUse);
		$this->assign('do_uses', $res['use']);
    ...
    public function add_info(){
    	...
    	$this->assign("douse", array('no'=>time().rand(10, 99), "username"=>$name, "admin_user_id"=>$id,
    			'deport_id'=>$deport_id, 'make_id'=>0, 'report_time'=>time()));
    	$this->display();
    }
    
    public function create_info(){
    	if( isset($_POST['douse']) ){
            $res = D('DoUse')->create_info($_POST['douse']);
            $res_list = D('DoUse')->create_info_item($_POST['entry'],$res[id],$_POST['douse']['admin_user_id']);
    		$notice = $res['msg'];
    		if( $res['status'] ){
    			$this->set_notice(1, $notice);
    			$this->redirect("DoUse/index");
                exit;
    		}else{
                $this->set_notice(2, $notice);
    			$this->assign('do_use', $_POST['do_use']);
    	...
    	$this->set_notice(2, $notice);
    	$this->redirect('DoUse/add_info');
    }
    
    public function edit(){
    	$this->assign("loc_name_2", "修改");
    	$do_use = M('do_uses')->alias("a")->join("left join ".C("DB_PREFIX")."admin_user b on b.id=a.admin_user_id")
    	...
    	$do_use['report_time']=time();
    	$this->assign("douse", $do_use);
    	$do_use_items = M('do_use_items')->where("do_use_id=%d", array($_GET['id']))->select();
    	$this->assign("do_use_items", $do_use_items);
    	$this->display();
    }
    ...
    public function detail(){
    	...
    	if( isset($_GET['id']) ){
    		$douse = M('do_uses')->alias("a")->join("left join ".C("DB_PREFIX")."admin_user b on b.id=a.admin_user_id")
    		->join("left join ".C("DB_PREFIX")."deport c on c.id=a.deport_id")
    		->field("a.*,ifnull(b.name, b.username) as username,c.name as deport_name")
    		->where("a.id=%d", array($_GET['id']))->find();
    		$this->assign("douse", $douse);
    		$do_use_items = M('do_use_items')->alias("a")->where("a.do_use_id=%d", array($_GET['id']))
    		->order("a.create_time desc")->select();
    		$this->assign("do_use_items", $do_use_items);
                 $do_use_status = M('do_use_status')->alias("a")
                                        ...
                                        ->where("a.do_use_id=%d", array($douse['id']))
                                        ->order("a.create_time desc")->select();
	    	$this->assign("do_use_status", $do_use_status);
    ...
    public function update_info(){
    	if( isset($_POST['douse']) ){
    		$res = D('DoUse')->update_info($_POST['douse']);
    		$res_list = D('DoUse')->update_info_item($_POST['entry'],$res[id],$_POST['douse']['admin_user_id']);
    		$notice = $res['msg'];
    		if( $res['status'] ){
    			$this->set_notice(1, $notice);
    			$this->redirect("DoUse/index");
    	...
    	$this->set_notice(2, $notice);
    	$this->redirect('DoUse/edit', array('id'=>$_POST['douse']['id']));
    }
    
    public function del(){
    	$f = M('do_uses')->where("id=%d", array($_GET['id']))->setField(array('is_del'=>1));
    	if( $f!==false ){
    ...
    public function check(){
    	if( isset($_POST['check_id']) ){
    		if( D('DoUse')->is_checked_power($_SESSION['admin_user_id'], $_POST['check_id']) ){
	    		if( isset($_POST['type']) && $_POST['type']=='checked' ){
	    			$status = 1;
	    		}else{
	    			$status = 2;
	    		}
	    		$process_type_status = D('DoUse')->get_status($_POST['check_id']);
	    		...
	    		$f = M('do_use_status')->add(array("do_use_id"=>$_POST['check_id'],
	    		...
	    		if( $pts['id']==$process_type_status['id'] ){
	    			$t = M('do_uses')->where("id=%d", array($_POST['check_id']))->setField(array("is_over"=>1));
	    		}
    ...
    $douse_status = M('do_use_status')->alias("a")
                                            ->join("left join ".C("DB_PREFIX")."admin_user b on b.id=a.admin_user_id")
                                            ...
                                            ->where("a.do_use_id=%d", array($_GET['id']))->order("a.create_time asc")->select();
    $this->assign('douse_status', $douse_status);
    $this->display("DoUse/_checked_list");
    exit;
    }
