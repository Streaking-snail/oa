...
class LeaveController extends AdminBaseController {
	...
	function __construct(){
		...
		$this->locations = array(
				array('is_checked'=>false, 'name'=>'广博集团请假单', 'url'=>U('Leave/index'))
		);
       $this->assign('loc_name_1', "广博集团请假单");
    ...
    public function add_info(){
    	...
    	$this->assign("leave", array('no'=>time().rand(10, 99), "username"=>$name,"admin_user_id"=>$id,
    ...
    public function create_info(){
    	if( isset($_POST['leave']) ){
            $res = D('Leave')->create_info($_POST['leave']);
    		$notice = $res['msg'];
    		if( $res['status'] ){
    			$this->set_notice(1, $notice);
    			$this->assign('leave', $_POST['leave']);
    	...
    	$this->set_notice(2, $notice);
    	$this->redirect('Leave/add_info');
    }
    
    public function edit(){
    	$this->assign("loc_name_2", "修改");
    	$leave = M('leaves')->alias("a")->join("left join ".C("DB_PREFIX")."admin_user b on b.id=a.admin_user_id")
    ...
    public function update_info(){
    	if( isset($_POST['leave']) ){
    		$res = D('Leave')->update_info($_POST['leave']);
    		$notice = $res['msg'];
    		if( $res['status'] ){
    			$this->set_notice(1, $notice);
    			$this->redirect("Leave/index");
    	...
    	$this->set_notice(2, $notice);
    	$this->redirect('Leave/edit', array('id'=>$_POST['leave']['id']));
    }
    ...
    public function detail(){
    	...
    	if( isset($_GET['id']) ){
    		...
    		$this->assign("leave", $leave);
                ...
                $leaves_status = M('leaves_status')->alias("a")
                                        ...
                                        ->where("a.leaves_id=%d", array($leave['id']))
    ...
    public function del(){
    	$f = M('leaves')->where("id=%d", array($_GET['id']))->setField(array('is_del'=>1));
    	if( $f!==false ){
 ...
 //审核
    public function check(){
    	if( isset($_POST['check_id']) ){
    		if( D('Leave')->is_checked_power($_SESSION['admin_user_id'], $_POST['check_id']) ){
	    		...
	    		$process_type_status = D('Leave')->get_status($_POST['check_id']);
	    		...
	    		$f = M('leaves_status')->add(array("leaves_id"=>$_POST['check_id'],
	    		...
	    		if( $pts['id']==$process_type_status['id'] ){
	    			$t = M('leaves')->where("id=%d", array($_POST['check_id']))->setField(array("is_over"=>1));
	    		}
    ...
    function expUser(){//导出Excel
    	...
    	if( !empty($_POST['admin_user']) ){
    		$where .= " and b.username=".str_filter($_POST['admin_user']);
    	}
    	...
    	$arrs = M('leaves')->alias('a')->join("left join ".C("DB_PREFIX")."admin_user b on b.id=a.admin_user_id")
    	->join("left join ".C("DB_PREFIX")."deport c on c.id=a.deport_id")
