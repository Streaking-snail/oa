...
class DoDesignModel extends BaseModel{
	
	protected $tableName = 'do_designs';
...
//                        }
		...
		$do_designs = $this->alias("a")->join("left join ".C("DB_PREFIX")."admin_user c on a.admin_user_id=c.id")
                        ->join("left join ".C("DB_PREFIX")."deport d on c.deport_id=d.id")
                        ->field("a.*")->where($where)->limit($page->firstRow . ',' . $page->listRows)->select();
		return array('do_designs'=>$do_designs, 'show_page'=>$page->show());
	}
	//添加
	...
	public function update_info($options=array(),$items=array()){
		...
		$f = $this->where("id=%d",array($id))->setField($options);
                $f1 = M('do_design_items')->where('do_design_id=%d',$id)->delete();
                foreach( $items as $key => $val){
                    if( $val['order_time'] && $val['price'] && $val['num']){
                        $val['do_design_id']=$id;
                        $val['create_time'] = time();
...
//                    if( $val['order_time'] && $val['price'] && $val['num']){
//                        $val['order_time'] = strtotime($val['order_time']);
//                        $f1 = M('do_design_items')->where("id=%d",array($val['id']))->setField($val);
//                    }
//                }
		if( $f!==false ){
