...
class LeaveModel extends BaseModel{
	...
	public function search_info( $options=array() ){
		...
		$seach =M('leaves');
		$leave = $seach->alias("a")->join("left join ".C("DB_PREFIX")."admin_user b on b.id=a.admin_user_id")
		...
		return array('leave'=>$leave, 'show_page'=>$page->show());
	}
...
// 		exit;
		...
		$id = $options['id'];
		unset($options['id']);
		$f = M('leaves')->where("id=%d",array($id))->setField($options);
		if( $f!==false ){
	...
	public function get_status_content(){
		$status = M('leaves_status')->where("leaves_id=%d", array($id))->order("id desc")->find();
		if( $status ){
        ...
        //获取当前状态
	public function get_local_status( $leaves_id ){
		$process_type_status = M('process_type_status')->alias("a")
							   ->join("left join ".C('DB_PREFIX')."leaves_status b on b.process_type_status_id=a.id")
							   ...
							   ->where("b.leaves_id=%d", array($leaves_id))->order("b.id desc")->find();
		return $process_type_status;
	}
        
        //获取状态内容
	public function get_status($leaves_id){
		$process_type = M('process_types')->where("controller_name='Leave'")->find();
		$process_type_status = M('process_type_status')->alias("a")
							   ->join("left join ".C('DB_PREFIX')."leaves_status b on b.process_type_status_id=a.id")
							   ->join("left join ".C('DB_PREFIX')."admin_user au on au.id=a.checked_user_id")
							   ->field("a.id, a.rank, a.name, ifnull(au.name, au.username) as username, b.status, a.checked_user_id")
       ...
       public function get_checked_list(){
    	...
    	$temporary_status = M('leaves_status')->alias("a")
    	...
    	$this->display("Leaves/_checked_list");
    	exit;
    }
    	//判断否为审核权限
	public function is_checked_power($user_id, $leaves_id){
