...
class DoPropagandaModel extends BaseModel{
	
	protected $tableName = 'do_propagandas';
...
//                        }
		...
		$propagandas = $this->alias("a")->join("left join ".C("DB_PREFIX")."admin_user c on c.id=a.admin_user_id")
                        ->join("left join ".C("DB_PREFIX")."deport d on c.deport_id=d.id")
                        ->field("a.*")->where($where)->limit($page->firstRow . ',' . $page->listRows)->select();
		return array('do_propagandas'=>$propagandas, 'show_page'=>$page->show());
	}
	...
	public function create_info($options=array(),$items=array()){
		...
		$id = $this->add($options);
                foreach( $items as $key => $val){
                    if( $val['order_time'] && $val['price'] && $val['num']){
                        $val['do_propaganda_id']=$id;
                        $val['create_time'] = time();
                        $val['admin_user_id'] = $_SESSION['admin_user_id'];
                        $val['order_time'] = strtotime($val['order_time']);
                        $ids = M('do_propaganda_items')->add($val);
                    }
                }
		if( $id>0 ){
	...
	public function update_info($options=array(),$items=array()){
		...
		$f = $this->where("id=%d",array($id))->setField($options);
                $f1 = M('do_propaganda_items')->where('do_propaganda_id=%d', $id)->delete();
                foreach( $items as $key => $val){
                    if( $val['order_time'] && $val['price'] && $val['num']){
                        $val['do_propaganda_id']=$id;
                        $val['create_time'] = time();
...
//                        $val['admin_user_id'] = $_SESSION['admin_user_id'];
//                        $val['order_time'] = strtotime($val['order_time']);
//                        $ids = M('do_propaganda_items')->where("id=%d", array($val['id']))->setField($val);
//                    }
//                }
		if( $f!==false ){
